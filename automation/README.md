...
> **前提**: `bash scripts/start_mlx_vlm_server.sh` でサーバーを起動しておく。
...
`mlx_vlm_history.py` の CLI は、保存画像に **full-image PaddleOCR** をかけたうえで、抽出したテキスト候補だけを `mlx_vlm.server` に渡して日付座標を特定する。`click_history()` も同じ認識アルゴリズムを使う。その後、修正ボタンのテンプレートマッチングも実行して座標を表示する（クリックはしない）。
...
- `screen_analyzer.py`: OCR integration (PaddleOCR / EasyOCR / PP-Structure helpers with caching)
