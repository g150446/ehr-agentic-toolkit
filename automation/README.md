...
4文字を超える文章や助詞を含む文は `type_japanese_sentence()` で文節単位に分割して**逐次入力**します。日本語全角で表示する記号（`、` → `,` + Enter / `。` → `.` + Enter / `・` → `/` + Enter / `ー` → `-` + Enter / `〜` → `~` + Enter / `「` `」` `『` `』` → 日本語モード bracket + Enter）に加えて、改行・`[` `]` `(` `)` `%` `:` も専用キー送信に切り替えて処理します。
