...
2. RapidOCR で日付候補 `(text, x, y)` を抽出し、`mlx_vlm.server` 上の Qwen 3.5 4B に **画像 + 候補一覧 + 対象日付** を渡して対象候補を特定
   - 正規表現で一意に特定できる場合はVLM不要（高速パス）
    - 同日候補が複数ある場合は、デフォルトで最上段の候補を選択
   - `mlx_vlm_history.py` と同じ full-image RapidOCR ベースの認識アルゴリズムを使う
...
`mlx_vlm_history.py` の CLI は、保存画像に対して **full-image RapidOCR** を実行し、抽出した候補一覧と画像自体を `mlx_vlm.server` に渡して日付座標を特定する。`click_history()` も同じ認識アルゴリズムを使う。その後、修正ボタンのテンプレートマッチングも実行して座標を表示する（クリックはしない）。
...
保存画像に対して、**OCRアンカーで過去カルテ列 ROI を推定**しつつ、RapidOCR / UI detection / EasyOCR を比較する。

```bash
# RapidOCR / UI detection / EasyOCR 比較
...
1. `RapidOCR + full-image OCR`
2. `RapidOCR + UI detection OCR`
3. `EasyOCR + full-image OCR`
...
- **OCR** - RapidOCR as default backend
...
|---------|---------|-------|----------------|
| `rapidocr` (default) | RapidOCR | Fast | Built-in ONNX recognition pipeline |
| `easyocr` | EasyOCR + PyTorch | Slower | Built-in ja+en model |

Both backends cache the loaded reader in memory so subsequent calls within the same process have near-zero initialization overhead.
...
# OCR
OCR_BACKEND=rapidocr      # rapidocr (default) or easyocr
OCR_LANGUAGES=ja,en       # Used by rapidocr / easyocr
...
Install RapidOCR in the active virtual environment:
```bash
...
- `ehr_input.py`: EHR field input automation (`open_test_patient_chart`, `close_record`, `click_history`, `edit_history`, `input_text_to_field`, `type_kanji_via_ime`, `type_japanese_sentence`, `detect_ime_mode`, `ensure_ime_mode`)。過去カルテ日付検出は `mlx_vlm_history.py` と同じ full-image RapidOCR ベースの認識アルゴリズムを使う
- `screen_analyzer.py`: OCR integration (RapidOCR / EasyOCR helpers with caching)
...
- `mlx_vlm_history.py`: `mlx_vlm.server` を使う過去カルテ日付検出（`click_history()` が使用）。RapidOCR 候補と画像自体を Qwen に渡し、OCR崩れを許容する正規表現で候補抽出しつつ、複数一致時は最上段を優先し、それでも候補が絞れないときだけ LLM を使う。CLI実行時は修正ボタンのテンプレートマッチングも実施
