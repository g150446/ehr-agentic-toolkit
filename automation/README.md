...
2. VLM（`mlx-community/gemma-4-e2b-it-4bit`）でOCRテキストを解析して対象日付を特定
   - 正規表現で一意に特定できる場合はVLM不要（高速パス）
   - VLMサーバー未起動の場合は正規表現フォールバックを使用
3. 検出した座標にBLEマウスを移動してクリック

> **VLM使用時の前提**: `bash scripts/start_mlx_vlm_server.sh` でサーバーを起動しておく。
> サーバーがない場合は正規表現フォールバックが自動的に使用される。
...
| 環境変数 | デフォルト | 説明 |
|---------|-----------|------|
| `MLX_VLM_HISTORY_URL` | `http://localhost:8181/v1/chat/completions` | mlx_vlm サーバーURL |
