...
> OCR を使うコマンドの前には `./scripts/start_ocr_server.sh` も別ターミナルで起動する。
...
`mlx-community/gemma-4-e2b-it-4bit` を使った LLM ベースの実装です。`ehr_input.py` では使用していませんが、LLM の出力比較などに利用できます。事前に `bash scripts/start_mlx_vlm_server.sh` でサーバーを起動してください。名前に `mlx_vlm` を含みますが、この用途では画像は送らずテキストだけを `/v1/chat/completions` に渡します。
...
2. `mlx_vlm.server` 上の `gemma-4-e2b-it-4bit` に OCR テキストだけを渡して対象日付を特定
   - 正規表現で一意に特定できる場合はVLM不要（高速パス）
   - 同日候補が複数ある場合は、デフォルトで最上段の候補を選択
   - `mlx_vlm_history.py` と同じ full-image PaddleOCR ベースの認識アルゴリズムを使う
   - OCR 自体は常駐 `ocr_server.py` にリクエストし、PaddleOCR モデルを毎回再初期化しない
3. 検出した座標にBLEマウスを移動してクリック

> **前提**: `./scripts/start_ocr_server.sh` と `bash scripts/start_mlx_vlm_server.sh` を起動しておく。
> この経路では画像自体は送らず、OCRで抽出した候補テキストだけを送信する。
...
| 環境変数 | デフォルト | 説明 |
|---------|-----------|------|
| `MLX_VLM_HISTORY_URL` | `http://localhost:8181/v1/chat/completions` | text-only chat completion endpoint (`mlx_vlm.server`) |
| `MLX_VLM_HISTORY_MODEL` | `mlx-community/gemma-4-e2b-it-4bit` | 使用モデル |
| `MLX_VLM_HISTORY_TIMEOUT` | `120` | タイムアウト秒数 |
| `OCR_SERVER_SOCKET_PATH` | `/tmp/paddle_ocr_server.sock` | OCR サーバーの Unix ソケット |
| `OCR_SERVER_TIMEOUT` | `120` | OCR サーバー応答待ち秒数 |
| `OCR_SERVER_DEVICE` | `auto` | Paddle 実行デバイス。現状の macOS ビルドでは CPU のみ |
...
`mlx_vlm_history.py` の CLI は、保存画像を常駐 OCR サーバーへ送り **full-image PaddleOCR** を実行したうえで、抽出したテキスト候補だけを `mlx_vlm.server` に渡して日付座標を特定する。`click_history()` も同じ認識アルゴリズムを使う。その後、修正ボタンのテンプレートマッチングも実行して座標を表示する（クリックはしない）。
...
保存画像に対して、**OCRアンカーで過去カルテ列 ROI を推定**しつつ、PaddleOCR / UI detection / PP-Structure を比較する。

```bash
# PaddleOCR / UI detection / PP-Structure 比較
...
1. `PaddleOCR + full-image OCR`
2. `PaddleOCR + UI detection OCR`
3. `PP-StructureV3 + Paddle OCR`
...
- **OCR** - PaddleOCR as default backend
...
1. PaddleOCR / EasyOCR runs on the full image at once
...
2. **Word-split OCR fallback** — if YOLO finds no elements (e.g. Windows desktop apps), PaddleOCR runs with word boxes (`return_word_box=True`) and uses those to keep text segments smaller.
...
|---------|---------|-------|----------------|
| `paddleocr` (default) | PaddleOCR | Standard | PP-OCRv4 where supported, otherwise PP-OCRv5 |
| `easyocr` | EasyOCR + PyTorch | Slower | Built-in ja+en model |

PaddleOCR downloads the required detection/recognition models on first run and caches them. Both backends cache the loaded reader in memory so subsequent calls within the same process have near-zero initialization overhead.
...
# OCR
OCR_BACKEND=paddleocr     # paddleocr (default) or easyocr
OCR_LANGUAGES=ja,en       # Used by paddleocr / easyocr
OCR_USE_GPU=false         # easyocr only; auto-set to true on Apple Silicon (MPS)
OCR_SERVER_SOCKET_PATH=/tmp/paddle_ocr_server.sock
OCR_SERVER_TIMEOUT=120
OCR_SERVER_DEVICE=auto    # current macOS Paddle build falls back to CPU
...
Install PaddleOCR and PaddlePaddle in a supported Python environment:
```bash
...
OCR を使う automation コマンドの前に、別ターミナルで OCR サーバーを起動してください:
...
- `ocr_server.py`: Long-running OCR server (Unix socket, keeps PaddleOCR preloaded across requests)
- `ocr_client.py`: Sync client for `ocr_server.py`
- `ble_client.py`: Sync client for `ble_server.py`
- `ble_test_cli.py`: Interactive BLE testing CLI tool
- `ehr_input.py`: EHR field input automation (`open_test_patient_chart`, `close_record`, `click_history`, `edit_history`, `input_text_to_field`, `type_kanji_via_ime`, `type_japanese_sentence`, `detect_ime_mode`, `ensure_ime_mode`)。過去カルテ日付検出は `mlx_vlm_history.py` と同じ full-image PaddleOCR ベースの認識アルゴリズムを使う
- `screen_analyzer.py`: OCR integration (PaddleOCR / EasyOCR / PP-Structure helpers with caching). `ocr_server.py` が内部で使用
...
- `mlx_vlm_history.py`: `mlx_vlm.server` を text-only chat completion endpoint として使う過去カルテ日付検出（`click_history()` が使用）。OCR崩れを許容する正規表現で候補抽出し、複数一致時は最上段を優先し、それでも候補が絞れないときだけ LLM を使う。CLI実行時は修正ボタンのテンプレートマッチングも実施
