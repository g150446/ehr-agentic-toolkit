...
現在の `ehr_input` は、長文入力時に **Qwen 3.5 4B MLX を優先して日本語セグメントを切り出し**、ローマ字化はローカル辞書で補正しながら逐次入力します。IME 候補確認も Qwen 優先で行い、一致未確認の候補を盲目的に Enter で確定しないようにしています。**カタカナ部分は mixed セグメント中でも切り出して、常に F7 による全角カタカナ変換で確定**するため、`アレルギー性` のような語でもカタカナ部分を漢字変換候補に流しません。BLE で扱いにくい記号は入力前に可読な代替へ正規化しており、たとえば `℃` は `C`、`×` は `x` として送信します。さらに helper reset では、**コマンド開始時に patient_record の第3ペイン座標を 1 回だけ検出して VLM 検証し、以後の Escape compare では同じ crop 座標を再利用**します。比較基準の文字列側も、**最後の日本語 anchor に直後の確定済み ASCII/記号 suffix を連結した anchor tail**（例: `症状(`）を保持するため、Escape 後にその suffix が残っていても正常状態として扱います。

日本語全角でそのまま表示したい記号は、現在 **`、` `。` `・` `ー` `〜` `「` `」` `『` `』`** を特別扱いしています。`、` `。` `・` `〜` `「` `」` `『` `』` は前後の単語から**単独トークンとして分離**し、日本語モードで対応キーを送った直後に **Enter で明示確定**します。**長音記号 `ー` は単独で現れる場合だけ同様に即時確定し、ひらがな・カタカナの直後に続く場合はその語の一部として保持**して一緒に変換・確定します（例: `コーテフ`, `えーと`, `アレルギー`）。その他の全角記号（例: `（` `）` `％` `：` `［` `］` `【` `】`）は、今のところ **半角 ASCII に正規化して送信**します。また `→` → `->`、`⇒` → `=>`、`℃` → `C`、`×` → `x` のように医療文脈で慣用的な ASCII 表記へ変換する多文字置換も行います。
...
| `--openrouter [model]` | 文節分割・IME 候補読取・ヘルパー単語提案を OpenRouter の vision 対応モデルへ切り替える。単独利用ではモデル指定必須 |
| `--openrouter --novita [model]` | OpenRouter と Novita を eligible な VLM リクエストごとに交互に使う。両方とも同じモデル ID を使い、省略時は `google/gemma-4-31b-it` |
