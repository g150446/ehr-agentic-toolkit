...
def build_segmentation_prompt(text: str) -> str:
    ...
    return (
        "以下の日本語の文章を「語単位」に分割してください。\n\n"
        "ルール:\n"
        "1. 「漢字だけ」で意味が通る語（例：維持、終了、酸素化、点滴、内服、著効）は、漢字部分を1単位として分割し、ひらがなとは別に扱う。\n"
        "   例:\n"
        '   - 「維持」→ 1単位（「維持でき」ではなく「維持」だけで分割）\n'
        '   - 「終了」→ 1単位（「終了とし」ではなく「終了」と「とし」を分ける）\n\n'
        '2. 「買う」「する」「切る」「かける」など、漢字だけでは読みや意味が完成しない語は、漢字とその直後のひらがなを1単位としてまとめる。\n'
        "   例:\n"
        '   - 「買う」→ 「買う」を1単位（「買」だけではなく「買う」全体）\n'
        '   - 「切り替える」→ 「切り替える」を1単位\n'
        '   - 「して」「しては」「されて」など、活用語尾込みで意味が成立するものも、漢字＋ひらがなを1単位で扱う\n\n'
        "3. 動詞・形容詞の活用語尾（〜いる、〜ている、〜た、〜ないなど）は、漢字部分とは別単位として分割する。\n"
        "   例:\n"
        '   - 「している」→ 「して」と「いる」\n'
        '   - 「できている」→ 「でき」と「ている」\n\n'
        "4. 助詞・接続助詞・格助詞は、前の語とは別単位として分割する。\n"
        "   例:\n"
        '   - 「での」→ 「の」は助詞だが、「〜での」慣用表現なら「での」を1単位とすることも可\n'
        '   - 「は」「も」「と」などは単独単位\n\n'
        "5. 日本語の文節だけを返し、ASCII記号や改行は分割対象に含めない\n\n"
        "6. 複数の単語が結合されて1つの単語になっている複合語は、意味の通る構成単位に分割する。\n"
        "   例:\n"
        '   - 「気管支拡張薬」→ 「気管支」「拡張」「薬」\n'
        '   - 「動脈血ガス分析」→ 「動脈血」「ガス」「分析」\n\n'
