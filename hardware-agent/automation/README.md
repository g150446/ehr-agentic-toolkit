...
> `--openrouter [model]` は **文節分割・IME モード検出・候補読取・ヘルパー単語提案** を OpenRouter 側へ切り替えます。画像付き IME 読取も行うため、**vision 対応のモデル**を指定してください。単独利用ではモデル指定必須です。
...
> `--openrouter --novita [model]` を同時指定すると、**OpenRouter と Novita をリクエストごとに交互に使用**します。両方とも同じモデル ID を使い、モデル省略時は `google/gemma-4-31b-it` を共有します。
...
# OpenRouter と Novita を交互に使ってレートリミットを分散
python -m automation.ehr_input --openrouter --novita "両肺野に"

# OpenRouter / Novita の両方で同じ共有モデルを使う
...
# Google AI Studio の Gemma 4 26B A4B で文節分割・候補読取・ヘルパー単語提案を実行
...
日本語テキストを渡すと、`ehr_input.py` は **Qwen3-VL-8B-Instruct（mlx_vlm）を優先して文節分割**し、ローマ字はローカル辞書で補正してから IME 入力します。Qwen の分割が細かすぎて IME 候補を不安定化させる場合は、`sudachipy + pykakasi` のローカル分割へ自動フォールバックします。引数が読み取り可能なテキストファイルなら、その**ファイル内容**を同じ入力フローに流します。helper reset では、**コマンド開始時に patient_record の第3ペイン座標を 1 回だけ検出して VLM 検証し、その後は同じ座標を compare crop に再利用**します。比較文字列側も、**最後の日本語 anchor に直後の確定済み ASCII/記号 suffix を連結した anchor tail**（例: `症状(`）を baseline / compare に使い、Escape 後にその suffix が残っていても正常な reset 完了と判定します。

日本語全角でそのまま表示したい記号は、現在 **`、` `。` `・` `ー` `〜` `「` `」` `『` `』`** を特別扱いしています。`、` `。` `・` `〜` `「` `」` `『` `』` は前後の単語から**単独トークンとして分離**し、日本語モードで対応キーを送った直後に **Enter で明示確定**します。**長音記号 `ー` は単独で現れる場合だけ即時確定し、ひらがな・カタカナの直後ではその語の一部として保持**して一緒に変換・確定します（例: `コーテフ`, `えーと`, `アレルギー`）。その他の全角記号（例: `（` `）` `％` `：` `［` `］` `【` `】`）は、今のところ **半角 ASCII に正規化して送信**します。

各実行のログは `logs/*.txt` に保存され、先頭に **実行ファイル名・生のコマンドライン・解析済みオプション要約** が記録されます。

運用時に変換結果を確認するときは、`[VLM一致]`、`[候補照合/romaji]`、`[試行N]`、`[ヘルパー単語]` の行を追うと、候補番号の誤選択・候補未発見・フォールバック発火を切り分けやすくなります。pure kanji ターゲットでは、読みだけ一致する mixed 候補を即採用せず、より安全な後続確認へ回します。また、romaji 一致候補がターゲットより短い場合も別単語とみなし採用しません（例: "昭かな" で "明らかな" を誤選択する問題の防止）。helper reset compare の `[helper reset][compare]` では、**最初に固定した compare crop** と **最後の確定済み文字列** を基準に判定しており、`症状(` のような anchor tail がそのまま残っていれば追加文字扱いにはしません。

**かな↔漢字クロスタイプガード**: fuzzy matching（`_ime_candidate_matches`）で 1 文字差の候補を許容する際、差異がかな（U+3040-30FF）と漢字（U+4E00-9FFF）のクロスタイプの場合は OCR ノイズとみなさず即却下します（例: "直地に" → "直ちに" の誤マッチ防止）。漢字同士の差異（著名な→著明な など）は 3 文字以上の場合のみ許容します。

**2文字ファジー無効化**: 2 文字のターゲットではファジーマッチ（1 文字差許容）を無効にし、完全一致のみ受け付けます。2 文字では 50% の不一致を許容することになり、別単語の誤採用リスクが高いためです（例: "血競" で "血症" を誤選択 → 実際には "血漿" の問題の防止）。VLM 補完読取が代わりにトリガーされ、より正確な候補認識が期待できます。

**視覚的類似先頭文字（Pass 5）**: suffix 一致で先頭文字のみ異なる候補を受け入れる際、**先頭 2 文字が両方とも漢字**である場合のみ採用します（例: 署明な→著明な）。かな suffix を持つ動詞活用（例: "燈って" で "伴って" を誤採用）や非漢字先頭文字は視覚的類似とみなしません。
...
4文字を超える文章や助詞を含む文は `type_japanese_sentence()` で文節単位に分割して**逐次入力**します。日本語全角で表示する記号のうち、`、` → `,` + Enter / `。` → `.` + Enter / `・` → `/` + Enter / `〜` → `~` + Enter / `「` `」` `『` `』` → 日本語モード bracket + Enter は独立確定します。一方で **`ー` は単独トークンのときだけ `-` + Enter で即時確定し、ひらがな・カタカナの直後では語に含めたまま処理**します。改行・`[` `]` `(` `)` `%` `:` も専用キー送信に切り替えて処理します。
...
**IME候補の検出方法**: patient-record 画面では第3パネルに絞ったうえで、まず **PP-StructureV3** で候補ポップアップの表/リスト状領域を抽出し、失敗時のみ従来の OpenCV ベース（青い選択バー / 反転領域）crop にフォールバックする。PP-StructureV3 は初回実行時に公式モデルを自動ダウンロードすることがある。
...
**`detect_ime_mode(client, config)`**: `'a'` を1文字入力し、Qwen3-VL（omlx VLM サーバー）で画面を読み取って IME モードを検出する。英語入力モードなら `'a'` が、日本語（ひらがな）入力モードなら `'あ'` が表示される。判定後に Backspace で入力した文字を削除する。`--openrouter` 指定時は OpenRouter のモデル、`--novita` 指定時は Novita AI の指定モデル（省略時は `google/gemma-4-31b-it`）、`--openrouter --novita [model]` 指定時はその共有モデルで OpenRouter / Novita を交互に使用し、`--fireworks` 指定時は Fireworks AI の指定モデル、`--google-ai-studio` 指定時は Google AI Studio の `gemma-4-26b-a4b-it` を使用する。
