...
- ヘルパー単語 fallback に入る直前は、IME の未確定組成が前の確定済み文字を巻き込まないよう **Escape を 500ms 間隔で 4 回**送ってから候補問い合わせへ進みます。`logs/*.txt` に `IMEポップアップをキャンセル (Esc×4, 500ms間隔)...` が出ているか確認すると追跡しやすいです。
