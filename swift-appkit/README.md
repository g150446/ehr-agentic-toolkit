...
EHR-Agent のチャット入力欄に以下のコマンドを送信することで、EHR の操作を自動化できます。
...
過去診療録を全件スクロール読み取りし、AI が退院時サマリを生成してサマリフォームの本文欄に貼り付けます。

**動作フロー:**
1. 過去診療録ページが開いていない場合は自動で開く
2. 全診療録レコードをスクロールしながらスクリーンキャプチャ・VLM 抽出
3. 抽出データを元に AI が退院時サマリを生成
4. サマリフォームを開き、本文欄にサマリテキストを貼り付け
...
過去診療録を全件スクロール読み取りし、AI がかかりつけ医宛の入院に関する診療情報提供書（紹介状）を生成して情報提供書フォームの本文欄に貼り付けます。

**動作フロー:**
1. 過去診療録ページが開いていない場合は自動で開く
2. 全診療録レコードをスクロールしながらスクリーンキャプチャ・VLM 抽出
3. 抽出データを元に AI が診療情報提供書を生成（入院期間・入院理由・経過・退院時診断・退院時処方・今後の方針を含む）
4. 情報提供書フォームを開き、本文欄に生成テキストを貼り付け

**前提条件:** いずれのコマンドも、対象患者の EHR ページをブラウザで開いた状態で実行してください。
